...
function canConvert() {
  const anyFile = Object.values(state.files).some(Boolean);
  return anyFile;
}
 ...
 * - İnsan bölgesi korunur (sakal/kaş bozulmaz)
...
async function whitenBackgroundWithSegmentation(srcCanvas) {
  if (typeof SelfieSegmentation === "undefined") {
    throw new Error("SelfieSegmentation yüklenmedi");
  }
  ...
  return new Promise((resolve, reject) => {
    ...
    segmentation.onResults((results) => {
      try {
        ...
        // 1️⃣ Önce SADECE insanı çiz
        ctx.drawImage(srcCanvas, 0, 0);

        // 2️⃣ Maskeyi uygula → arka planı sil
        ctx.globalCompositeOperation = "destination-in";
        ctx.drawImage(results.segmentationMask, 0, 0, w, h);

        // 3️⃣ Arka plana BEYAZ ekle
...
async function makeBiometricJpeg(file, sizeStr, doWhiten) {
  ...
  if (doWhiten) {
    // MediaPipe ile gerçek arka plan beyazlatma
  ...
  try {
    ...
    for (const card of cards) {
      ...
      if (key === "biyometrik_foto") {
        ...
        if (!jpegBlob) {
          alert("Biyometrik JPEG oluşturulamadı.");
          continue;
        }
